...
import com.pao.csv.model.FirstAndLastNameComparator;
...
public class FileContentServiceImpl implements FileContentService {
	...
	private static void setNullFileContentList() {
	...
	@PostConstruct
	public void init() {
		log.info(">> Start to load Data from any.csv");
		Resource resource = resourceLoader.getResource("classpath:any.csv");
	...
	public Collection<FileContent> getAllCustomContents() {
		...
		Map<String, List<FileContent>> groupByInsuranceCompanyMap = groupContentByInsuranceCompanyJava8();
		for (String insuranceCompany : groupByInsuranceCompanyMap.keySet()) {
			List<FileContent> smallFileContents = this.getAllHighestVersionWithSameUserIdJava8(insuranceCompany);
	...
	private void loadDataFromCsv(InputStream is) {
		...
		if (EntityUtil.isValidList(fileContents)) {
			setNullFileContentList();//Re-set it to null when get new contents from CSV file by upload from UI or default CSV file
			save(fileContents);
		}
	}
	...
	private Map<String, List<FileContent>> groupContentByInsuranceCompanyJava8() {
	...
	private Map<String, FileContent> groupByUserIdWithHighestVersion(List<FileContent> fileContents) {
		Map<String, FileContent> groupByUserIdWithHighestVersionMap = fileContents.stream().collect(Collectors.groupingBy(FileContent::getUserId, Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparingInt(FileContent::getVersion)), Optional::get)));
		return groupByUserIdWithHighestVersionMap;
	}

	private List<FileContent> getAllHighestVersionWithSameUserIdJava8(String insuranceCompany) {
		List<FileContent> fileContents = new ArrayList<FileContent>();
		Map<String, List<FileContent>> groupByInsuranceCompanyMap = groupContentByInsuranceCompanyJava8();
